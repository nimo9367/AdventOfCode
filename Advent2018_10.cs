...
public class Advent2018_10
{
    private List<Coord> _inputData = new List<Coord>();
    private int _prevMaxX;
    private int _prevMaxY;

    internal void Solve()
    {
        string pattern = @".*<(.*),(.*)>.*<(.*),(.*)>";
#region input
        string input = @"position=< 21459,  32026> velocity=<-2, -3>
...
// position=<-3,  6> velocity=< 2, -1>";
        ...
        {
            _inputData.Add(new Coord { 
                X = int.Parse(m.Groups[1].Value.Trim()), 
                Y = int.Parse(m.Groups[2].Value.Trim()), 
                VX = int.Parse(m.Groups[3].Value.Trim()), 
                VY = int.Parse(m.Groups[4].Value.Trim())
            });
        }
        var gen = 0;
        while(gen < 100000)
        {
            ...
            var maxX = _inputData.Max(x => x.X) - minX;
            var maxY = _inputData.Max(x => x.Y) - minY;
            if(!(_prevMaxX < maxX && _prevMaxY < maxY && _prevMaxX != 0))
            {
                _prevMaxX = maxX;
                _prevMaxY = maxY;
               continue;
    ...
    private void Print()
    {
        ...
        for(int y = 0; y <= maxY; y++)
        {
            ...
            {
                var coord = _inputData.FirstOrDefault(c => c.X == x + minX && c.Y == y + minY);
                if(coord != null)
                    line += "#";
                else
                    line += ".";
